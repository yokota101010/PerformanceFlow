# New Project - 仕様書 (Database & Domain Schema)
...
| `発注先ID` | `VARCHAR(255)` |  | ✅ (`発注先.発注先ID`) |  | 独立 |  |
| `年月` | `DATE` |  |  |  | 独立 | - YYYY-MM-01形式。<br />- `作業契約ID` で紐づく `案件作業明細` の `開始日` から `終了日` までの期間と重なる期間があること。 |
...
| `要員ID` | `VARCHAR(255)` | ✅ | ✅ (`月別要員工数サマリ.要員ID`), ✅ (`要員.要員ID`) |  | 独立 |  |
